...
function drawSegment(segment) {
    ...
    const startX = segment.start.x * scaleFactor ;
    const startY = segment.start.y * scaleFactor ;
    const endX = segment.end.x * scaleFactor ;
    const endY = segment.end.y * scaleFactor ;
    ...
    if (segment === selectedSegment) {
        ...
        ctx.fillRect(
            poslabel.x * scaleFactor  - 10,
            poslabel.y * scaleFactor  - 12,
            70,
            15
        );
        ctx.fillStyle = 'white';
        ctx.font = '14px Arial';
        ctx.fillText(`${distance}`, poslabel.x * scaleFactor, poslabel.y * scaleFactor);
    }
}
